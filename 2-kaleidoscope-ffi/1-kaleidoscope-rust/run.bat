@echo off

rem Most of this file is me being silly with formating.
rem Only 3 commands are actually executed, plus 'cd' commands.
rem
rem     tree-sitter generate
rem     cargo build
rem     target\debug\kaleidoscope.exe
rem
rem The 'cd' commands are because tree-sitter is run from the 
rem src/tree-sitter-via folder. I'm running the .exe without 
rem cargo run for aesthetic purposes too. :')

set BOLD=[1m
set RED=[91m
set GREEN=[32m
set GRAY=[90m
set CYAN=[36m
set RESET=[0m

echo %GRAY%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%RESET%
echo %CYAN%Running 'tree-sitter generate'%RESET%
echo %GRAY%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%RESET%

cd src/compiler/tree-sitter-via
tree-sitter generate && (
    echo %GRAY%~~~~~~~~~~~~~~~~~~~~~%RESET%
    echo %CYAN%Running 'cargo build'%RESET%
    echo %GRAY%~~~~~~~~~~~~~~~~~~~~~%RESET%
    
    cd ../../..
    cargo build && (
        echo %GRAY%~~~~~~~~~~~~~~%RESET%
        echo %GREEN%Build complete%RESET%
        echo %GRAY%~~~~~~~~~~~~~~%RESET%
        cd target\debug
        kaleidoscope.exe
        cd ../..
    ) || (
        echo %GRAY%~~~~~~~~~~~~~~~~~~~~~%RESET%
        echo %RED%'cargo build' failed.%RESET%
        echo %GRAY%~~~~~~~~~~~~~~~~~~~~~%RESET%
    )
) || (
    cd ../../..

    echo %GRAY%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%RESET%
    echo %RED%'tree-sitter generate' failed.%RESET%
    echo %GRAY%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%RESET%
)
